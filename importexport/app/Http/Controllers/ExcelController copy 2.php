...
public function import(Request $request)
{
    ...
    if ($headers[1] != 'name'){
        return redirect()->back()->withErrors($headers[1].' field not metch with our database');
    } 
    if ($headers[2] != 'date'){
        return redirect()->back()->withErrors($headers[2].' field not metch with our database');
    } 
    if ($headers[3] != 'status'){
        return redirect()->back()->withErrors($headers[2].' field not metch with our database');
    } 
